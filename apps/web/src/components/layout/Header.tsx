...
const Header: React.FC = () => {
  ...
  return (
    <>
      <div
        className={cn(
          "fixed w-full h-16 flex justify-between items-center bg-header-background dark:bg-header-background-dark px-10 z-50",
        )}
      >
        {status ? (
          <>
            <div className={cn("lg:hidden")}>
              <button onClick={() => setIsOpen(!isOpen)}>
                ...
                <CiMenuKebab
                  fontSize="35px"
                  className={cn(
                    "text-text-300 dark:text-text-dark-100 cursor-pointer",
                  )}
                />{" "}
              </button>
            </div>
            <div id="logo" className={cn("hidden lg:block")}>
              <h1
                onClick={() => navigate("/")}
                className={cn(
                  "text-text-300 dark:text-text-dark-100 font-bold tracking-widest",
                )}
              >
                S-BRAIN
              </h1>
          ...
          <div className={cn("rounded-sm py-2 px-4 cursor-pointer")}>
            {status ? (
              <button
                ...
                className={cn("font-medium text-[#ff0000] cursor-pointer")}
              >
                Logout
              </button>
            ) : (
              <>
                <button
                  ...
                  className={cn(
                    "font-medium text-text-300 dark:text-text-dark-100 cursor-pointer",
                  )}
                >
                  Login
                </button>
