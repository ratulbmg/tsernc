...
export const registerUser = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, response, "User Registration successfully"));
  },
);
...
export const loginUser = asyncHandler(async (req: Request, res: Response) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, response, "User Login successfully"));
});

export const meAccount = asyncHandler(async (req: Request, res: Response) => {
  const respones = await authService.meAccount(
    req.headers.authorization as string,
  );
  res.status(200).json(new ApiResponse(200, respones, "User Details"));
});
