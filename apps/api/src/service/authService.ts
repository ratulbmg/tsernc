...
class AuthService {
  ...
  async registerUser(req: UserRegisterRequest): Promise<UserLoginResponse> {
    ...
    const isEmailExist = await repositoryWrapper.userRepository.findUser({
      email: req.email,
    });
    if (isEmailExist) {
    ...
    const token = await createToken(
      { name: req.name, uniqueId: req.uniqueId },
      "7d",
    );
    if (!token) {
  ...
  async loginUser(req: UserLoginRequest): Promise<UserLoginResponse> {
    ...
    const isUserExist = await repositoryWrapper.userRepository.findUser({
      email: req.email,
      password: req.password,
    });
    if (!isUserExist) {
    ...
    const token = await createToken(
      { name: isUserExist.name!, uniqueId: isUserExist.uniqueId! },
      "7d",
    );
    if (!token) {
